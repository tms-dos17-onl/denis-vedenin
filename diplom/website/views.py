...
def login_user(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.success(
                request, "Помилка входу. Будь ласка, спробуйте ще раз...")
...
def past(request):
    if request.user.is_authenticated:
        ...
        messages.success(
            request, "Ви маєте бути авторизованим, щоб побачити попередній код")
        return redirect("home")
...
def delete_all_past(request):
    if request.user.is_authenticated:
        ...
        messages.success(
            request, "Ви маєте бути авторизованим, щоб видалити історію")
    return redirect("past")
